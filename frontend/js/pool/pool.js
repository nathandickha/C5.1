...
function createStepGeometry(runLength, stepWidth, height, params, layout) {
  ...
  const plan = pos === "right"
    ? [[x0, y1], [x0, y0], [x1, y1]]
    : [[x0, y0], [x0, y1], [x1, y0]];

  const vertices = [
    plan[0][0], plan[0][1], z1,
    plan[1][0], plan[1][1], z1,
    plan[2][0], plan[2][1], z1,
    plan[0][0], plan[0][1], z0,
    plan[1][0], plan[1][1], z0,
    plan[2][0], plan[2][1], z0,
  ];
...
/* =====================================================
    ...
    if (stepCount > 0) {
      ...
      for (let s = 0; s < stepCount; s++) {
        ...
        const geo = createStepGeometry(stepRun, layout.width, h, this.params ?? params, layout);
