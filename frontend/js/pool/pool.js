...
function createStepGeometry(runLength, stepWidth, height, params, layout) {
  ...
  const size = THREE.MathUtils.clamp(
    Math.min(Number(runLength) || 0, Number(stepWidth) || 0) || DIAGONAL_STEP_MAX_SIZE,
    0.05,
    DIAGONAL_STEP_MAX_SIZE
  );
  const x0 = -size * 0.5;
  const x1 = size * 0.5;
  const y0 = -size * 0.5;
...
/* =====================================================
    ...
    if (stepCount > 0) {
      ...
      for (let s = 0; s < stepCount; s++) {
        ...
        const stepRun = (liveParams?.stepShape === "diagonal" && s !== 1 && layout.position !== "center") ? Math.min(layout.width, DIAGONAL_STEP_MAX_SIZE) : (s === 1 ? STEP_LENGTH * 2 : STEP_LENGTH);
        const geo = createStepGeometry(stepRun, layout.width, h, liveParams, layout);
        const step = new THREE.Mesh(geo, new THREE.MeshStandardMaterial());

        const x = s === 1
          ? axisStartWallX + stepRun * 0.5
          : axisStartWallX + STEP_LENGTH * (s + 0.5);
