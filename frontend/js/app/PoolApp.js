...
// --------------------------------------------------------------
  ...
  getStepWidthSliderMax() {
    // Diagonal corner steps are equal triangles. Keep their X/Y size capped
    // to the wall-backed bench/second-step run, which is preset to 600 mm.
    if (this.poolParams?.stepShape === "diagonal") return 0.6;
  ...
  syncStepWidthSliderLimit() {
    ...
    const isDiagonal = this.poolParams?.stepShape === "diagonal";
    ...
    let width = Number(this.poolParams.stepWidth);
    ...
    // In Diagonal Corner mode the step is an equal triangle, so width and
    // extension are the same dimension. Keep both UI controls in sync and
    // cap both at the 600 mm bench/ledge run.
  ...
  setupStepLayoutControls() {
    ...
    const updateButtons = () => {
      ...
      const shape = this.poolParams.stepShape === "diagonal" ? "diagonal" : "rectangle";
    ...
    document.querySelectorAll("[data-step-shape]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const next = btn.dataset.stepShape === "diagonal" ? "diagonal" : "rectangle";
        if (this.poolParams.stepShape === next) return;
        this.captureUndoState?.("Step shape");
        this.poolParams.stepShape = next;
        if (next === "diagonal") {
          this.poolParams.stepWidth = Math.min(Number(this.poolParams.stepWidth) || 0.6, 0.6);
        }
        this.syncStepWidthSliderLimit?.();
  ...
  previewStepWidthSlider(widthValue) {
    ...
    const isDiagonal = this.poolParams?.stepShape === "diagonal";
    const widthMax = isDiagonal ? Math.min(fullWidth, 0.6) : fullWidth;
    const narrowWidth = THREE.MathUtils.clamp(Number(widthValue) || (isDiagonal ? 0.6 : 0.9), 0.05, widthMax);
    ...
    steps.forEach((step) => {
      ...
      if (isDiagonal && idx !== 1 && position !== "center") {
        ...
        if (Number.isFinite(baseLen) && baseLen > 0) {
          // Equal triangle: Step Width controls both the Y width and X run.
          // Anchor to the current back edge while the live preview changes.
          const backEdgeX = step.position.x - baseLen * (step.scale?.x ?? 1) * 0.5;
          step.scale.x = targetWidth / baseLen;
          step.position.x = backEdgeX + targetWidth * 0.5;
        }
      }
    ...
    ids.forEach((id) => {
      ...
      slider.addEventListener("input", (e) => {
        ...
        if (id === "stepWidth") {
          ...
          // Diagonal Corner uses an equal triangle, so Step Width and
          // Step Extension must move together.
          if (this.poolParams?.stepShape === "diagonal") {
            const extensionSlider = document.getElementById("stepExtension");
            const extensionOutput = document.getElementById("stepExtension-val");
            if (extensionSlider) {
              extensionSlider.min = slider.min;
