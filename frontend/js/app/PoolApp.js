...
// --------------------------------------------------------------
  ...
  syncStepWidthSliderLimit() {
    ...
    if (isDiagonal) this.poolParams.stepExtension = width;
    ...
    steps.forEach((step) => {
      ...
      if (isDiagonal && idx !== 1 && position !== "center") {
        ...
        if (Number.isFinite(baseLen) && baseLen > 0) {
          ...
          const size = THREE.MathUtils.clamp(targetWidth, 0.05, 0.6);
  ...
  setupPoolSliders() {
    ...
    ids.forEach((id) => {
      ...
      slider.addEventListener("input", (e) => {
        ...
        if (id === "stepWidth" && this.poolParams?.stepShape === "diagonal") {
          this.poolParams.stepExtension = val;
        }
