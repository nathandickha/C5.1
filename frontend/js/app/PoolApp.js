...
// --------------------------------------------------------------
  ...
  setupStepExtensionSlider() {
    ...
    slider.addEventListener("input", () => {
      ...
      let selectedLeftEdge = wallX;
      if (selectedIndex !== 1) {
        const selectedLengthBeforeMove = getStepLength(this.selectedStep);
        selectedLeftEdge = this.selectedStep.position.x - selectedLengthBeforeMove * 0.5;
      }

      const selectedLength = getStepLength(this.selectedStep);
      this.selectedStep.position.x = selectedLeftEdge + selectedLength * 0.5;

      let runX = selectedLeftEdge + selectedLength;
      sortedSteps.forEach((step) => {
        const stepIndex = Number.isFinite(Number(step.userData?.stepIndex))
          ? Number(step.userData.stepIndex)
          : sortedSteps.indexOf(step);

        if (step === this.selectedStep || stepIndex <= selectedIndex) return;
