...
// --------------------------------------------------------------
  ...
  setupStepLayoutControls() {
    ...
    const rebuildForStepLayout = async (_reason) => {
      ...
      // width/extension live previews keep working after Left/Centre/Right changes.
  ...
  previewStepWidthSlider(widthValue) {
    ...
    const narrowWidth = THREE.MathUtils.clamp(Number(widthValue) || 0.9, 0.05, fullWidth);
    ...
    steps.forEach((step) => {
      ...
      const targetWidth = idx === 1 ? fullWidth : narrowWidth;
      ...
      step.position.y = getCenterY(targetWidth, idx === 1);
