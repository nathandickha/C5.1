...
// --------------------------------------------------------------
  ...
  getStepWidthSliderMax() {
    // Diagonal corner steps are equal triangles. Their cap is the current
    // second/full-width bench extension, not a fixed 600 mm value.
    if (this.poolParams?.stepShape === "diagonal") return this.getBench2ExtensionValue();
  ...
  syncStepWidthSliderLimit() {
    ...
    if (!Number.isFinite(width) || width <= 0) width = isDiagonal ? 0.45 : maxWidth;
  ...
  syncStepExtensionSliderForSelectedStep() {
    ...
    if (!slider || this.poolParams?.stepShape !== "diagonal") return;
  ...
  setupStepExtensionSlider() {
    ...
    slider.addEventListener("input", () => {
      ...
      if (selectedStepIndex === 1) {
        this.poolParams.bench2Extension = val;
        if (this.poolParams?.stepShape === "diagonal") {
          const capped = Math.min(this.getDiagonalStepSizeValue(), val);
