...
// --------------------------------------------------------------
  ...
  isEqualCornerStepShape() {
    return this.poolParams?.stepShape === "diagonal" || this.poolParams?.stepShape === "circular";
  }
  ...
  syncStepExtensionSliderForSelectedStep() {
    ...
    if (!slider || !this.isEqualCornerStepShape?.()) return;
  ...
  setupStepLayoutControls() {
    ...
    const updateButtons = () => {
      ...
      const shape = (this.poolParams.stepShape === "diagonal" || this.poolParams.stepShape === "circular") ? this.poolParams.stepShape : "rectangle";
    ...
    document.querySelectorAll("[data-step-shape]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const next = (btn.dataset.stepShape === "diagonal" || btn.dataset.stepShape === "circular") ? btn.dataset.stepShape : "rectangle";
        if (this.poolParams.stepShape === next) return;
        this.captureUndoState?.("Step shape");
        this.poolParams.stepShape = next;
        if (next === "diagonal" || next === "circular") {
          const cap = this.getBench2ExtensionValue();
